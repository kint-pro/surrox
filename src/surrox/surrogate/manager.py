...
from surrox.surrogate.feature_reduction import FeatureGroup, FeatureReduction
...
class SurrogateManager:
    ...
    def save(self, path: Path) -> None:
        ...
        for column, surrogate in self._surrogates.items():
            ...
            feature_reduction_data = None
            if ensemble.feature_reduction is not None:
                feature_reduction_data = {
                    "selected": list(ensemble.feature_reduction.selected),
                    "dropped": list(ensemble.feature_reduction.dropped),
                    "groups": [
                        {
                            "original_names": list(g.original_names),
                            "combined_name": g.combined_name,
                            "center": g.center.tolist(),
                            "scale": g.scale.tolist(),
                            "components": g.components.tolist(),
                        }
                        for g in ensemble.feature_reduction.groups
                    ],
                }
            ...
            metadata["columns"][column] = {
                ...
                "feature_reduction": feature_reduction_data,
            }
    ...
    def load(cls, path: Path) -> SurrogateManager:
        ...
        for column, col_meta in metadata["columns"].items():
            ...
            fr_data = col_meta.get("feature_reduction")
            feature_reduction = None
            if fr_data is not None:
                feature_reduction = FeatureReduction(
                    selected=tuple(fr_data["selected"]),
                    dropped=tuple(fr_data["dropped"]),
                    groups=tuple(
                        FeatureGroup(
                            original_names=tuple(g["original_names"]),
                            combined_name=g["combined_name"],
                            center=np.array(g["center"], dtype=np.float64),
                            scale=np.array(g["scale"], dtype=np.float64),
                            components=np.array(g["components"], dtype=np.float64),
                        )
                        for g in fr_data["groups"]
                    ),
                )
